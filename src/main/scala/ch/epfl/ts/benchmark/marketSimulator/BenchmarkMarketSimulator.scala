...
class BenchmarkMarketSimulator(marketId: Long, rules: MarketRules) extends MarketSimulator(marketId, rules) {
  ...
  override def receiver = {
    case last: LastOrder => {
      if (!askOrdersBook.isEmpty && !bidOrdersBook.isEmpty) {
        send(FinishedProcessingOrders(askOrdersBook.size, bidOrdersBook.size, askOrdersBook.head.price - bidOrdersBook.head.price))
      } else {
        send(FinishedProcessingOrders(0,0,0.0))
      }
    }
