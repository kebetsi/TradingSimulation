...
import scala.collection.mutable.{TreeSet => MTreeSet, HashMap => MHashMap}
...
class PartialOrderBook(val comparator: Ordering[Order]) {
  ...
  def delete (o: Order): Boolean = bookMap remove o.oid map { r => book remove r; true } getOrElse { false }
