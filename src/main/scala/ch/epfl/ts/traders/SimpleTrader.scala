  ...
  override def receiver = {
    ...
    case SendMarketOrder => {
      if (alternate % 2 == 0) {
        ...
        send[Order](new MarketBidOrder(orderId, uid, System.currentTimeMillis(), USD, USD, 50, 0))
      } else {
        println("SimpleTrader: sending market ask order")
        send[Order](new MarketAskOrder(orderId, uid, System.currentTimeMillis(), USD, USD, 0, 50))
      }
