...
class BitstampTransactionPullFetcher(marketId: Long) extends PullFetch[Transaction] {
  val bitstamp = new BitstampAPI(marketId, USD, BTC)
  var count = 100
...
class BitstampAPI(marketId: Long, from: Currency, to: Currency) {
  ...
  def getTrade(count: Int): List[Transaction] = {
    ...
    t.map(f => new Transaction(marketId, f.price.toDouble, f.amount.toDouble, f.date.toLong * 1000, BTC, USD, 0, 0, 0, 0))
  }
  ...
  def getDepth(count: Int): List[LimitOrder] = {
    ...
    try {
      ...
      val asks = a.asks match {
        case l: List[List[String]] => l.map { e => LimitAskOrder(0, marketId, System.currentTimeMillis, from, to, e.last.toDouble, e.head.toDouble) }
        case _                     => List[LimitOrder]()
      }
      val bids = a.bids match {
        case l: List[List[String]] => l.map { e => LimitBidOrder(0, marketId, System.currentTimeMillis(), from, to, e.last.toDouble, e.head.toDouble) }
        case _                     => List[LimitOrder]()
      }
      t = asks ++ bids
    } catch {
